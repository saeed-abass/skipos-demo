        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            <div className="flex flex-col items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-soft-text">No waste transfer notes yet</p>
                <p className="mt-0.5 text-xs text-soft-muted">Create your first WTN to start building your compliance record</p>
              </div>
              <button
                onClick={onNew}
                className="mt-1 inline-flex items-center gap-1.5 rounded-btn bg-gradient-orange px-4 py-2 text-[0.65rem] font-bold uppercase tracking-[0.025em] text-white shadow-soft hover:shadow-md transition-all"
          ...
          <div className="divide-y divide-gray-50">
            {visible.map(wtn => (
              <div
                ...
                className="flex cursor-pointer flex-col gap-1.5 p-4 transition-colors hover:bg-orange-50/30"
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-mono text-xs font-semibold text-soft-text">{wtn.wtn_number}</span>
                  <WTNStatusBadge status={wtn.status as WTNStatus} />
                </div>
                <p className="font-medium text-soft-text">{wtn.job.customer.name}</p>
                <p className="line-clamp-1 text-xs text-soft-muted">{truncate(wtn.waste_description, 50)}</p>
                {wtn.transfer_date && (
                  <p className="text-xs text-soft-muted">{formatDate(wtn.transfer_date)}</p>
                )}
              </div>
            ))}
          </div>
